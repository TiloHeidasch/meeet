...
// (issues #72 and #90). Exactly three uninstrumented Node 24 children supply
...
// validation, and response serialization. Routing-window diagnostics are
// collected by the timing workers after both requests, never by the parent.
...
function run(heapSnapshots: boolean, cpuProfile: boolean): void {
  if (!existsSync(WORKER_PATH)) throw new Error("The profiling worker is missing.");
  ...
  const reportPath = resolvePath(PROFILES_DIR, `report-${aggregate.artifact.compiledArtifactId.slice(0, 12)}-${new Date().toISOString().replaceAll(":", "-")}.json`);
  writeFileSync(reportPath, `${JSON.stringify(aggregate, null, 2)}\n`);
  process.stdout.write(`${reportPath}\n`);
  printRankedTable(aggregate);
}
...
export function hasExposeGc(): boolean {
  const nodeGlobal = globalThis as typeof globalThis & { gc?: () => void };
  return typeof nodeGlobal.gc === "function";
}
