...
import RouteFirstOverview from "./RouteFirstOverview";
...
type ProviderMode = "fixture" | "configured" | "mvg-direct-transit" | "self-hosted-routing";
type RoutingFoundation = { state: "configured-foundation"; calculationAvailable: false; reason: "calculation-not-migrated" };
export type PlannerCapability = { mode: ProviderMode; supportedModes: Mode[]; routingFoundation?: RoutingFoundation };
...
const CONFIGURED_FOUNDATION_MESSAGE = "Self-hosted routing foundation is configured, but meeting calculation requests are unavailable and will not be sent. Location search remains available for choosing Munich starting points.";

export function getPlannerUiState(capability: PlannerCapability): PlannerUiState {
  const calculationUnavailable = capability.routingFoundation?.calculationAvailable === false;
  return {
    calculationUnavailable,
    canCalculate: !calculationUnavailable,
    controlsDisabled: calculationUnavailable,
    showModeSelector: !calculationUnavailable,
    unavailableMessage: CONFIGURED_FOUNDATION_MESSAGE,
  };
}

export function canSubmitMeetingCalculation(plannerUiState: PlannerUiState, status: "idle" | "loading" | "error" | "success" | "empty") {
  return plannerUiState.canCalculate && status !== "loading";
}
...
export function mapParticipantSource<T>(routeFirstActive: boolean, legacySnapshot: readonly T[] | undefined, currentParticipants: readonly T[]): readonly T[] {
  return routeFirstActive ? currentParticipants : legacySnapshot ?? currentParticipants;
}
...
function MapCanvas({ calculation, participants: currentParticipants, selectedPoiId, onPoiSelect, routeLegs, showModes, routeFirstEvidence }: { calculation: CalculationState | null; participants: readonly Participant[]; selectedPoiId: string | null; onPoiSelect: (id: string) => void; routeLegs: readonly RouteLeg[]; showModes: boolean; routeFirstEvidence: RouteFirstMapEvidence | null }) {
  const response = calculation?.response;
  const routeFirstActive = Boolean(routeFirstEvidence);
  ...
  const currentMapParticipants = currentParticipants.flatMap((participant) => participant.location ? [{ id: participant.id, location: { label: participant.location.label, latitude: participant.location.lat, longitude: participant.location.lng }, mode: participant.mode }] : []);
  const participants: MapParticipant[] = mapParticipantSource(routeFirstActive, snapshotParticipants, currentMapParticipants).map((requestParticipant, index) => {
    const submitted = calculation?.submittedParticipants.find((participant) => participant.id === requestParticipant.id);
    return { id: requestParticipant.id, number: index + 1, label: submitted?.name || `Participant ${index + 1}`, mode: showModes ? MODE_SHORT[requestParticipant.mode] : "Mode not enabled", latitude: requestParticipant.location.latitude, longitude: requestParticipant.location.longitude, color: COLORS[index] };
  });
  const pois: MapPoi[] = routeFirstActive ? [] : response?.status === "ok" ? response.pois.map((poi, index) => ({ ...poi, number: index + 1 })) : [];
  return <MapLibreCanvas corridor={routeFirstActive ? undefined : response?.status === "ok" ? response.corridor : undefined} participants={participants} pois={pois} routeLegs={routeFirstActive ? [] : routeLegs.map((leg, index) => ({ participantId: leg.participantId, color: COLORS[index] ?? COLORS[0], geometry: leg.geometry }))} routeFirstEvidence={routeFirstEvidence} selectedPoiId={selectedPoiId} onPoiSelect={onPoiSelect} resultState={resultState} />;
}

function ParticipantRow({ participant, index, locationError, locationRef, onChange, onRemove, canRemove, disabled, supportedModes, showModeSelector }: { participant: Participant; index: number; locationError?: string; locationRef: (element: HTMLInputElement | null) => void; onChange: (next: Participant) => void; onRemove: () => void; canRemove: boolean; disabled: boolean; supportedModes: readonly Mode[]; showModeSelector: boolean }) {
...
export default function MeetPlanner({ capability }: { capability: PlannerCapability }) {
  ...
  const plannerUiState = getPlannerUiState(capability);
  const { calculationUnavailable } = plannerUiState;
  ...
  const [routeFirstEvidence, setRouteFirstEvidence] = useState<RouteFirstMapEvidence | null>(null);
  ...
  const handleRouteFirstEvidence = useCallback((next: RouteFirstMapEvidence | null) => setRouteFirstEvidence(next), []);
  ...
  async function calculate() {
    if (!canSubmitMeetingCalculation(plannerUiState, status)) return;
  ...
  return <main className="min-h-screen bg-[#f4f1eb] text-[#202522]"><AttributionSummary metadata={calculation?.response.metadata} /><RouteFirstOverview key={`${tolerance}-${participants.map((participant) => `${participant.id}:${participant.location?.lat ?? ""},${participant.location?.lng ?? ""}:${participant.mode}`).join("|")}`} participants={participants.map((participant) => ({ id: participant.id, location: participant.location ? { latitude: participant.location.lat, longitude: participant.location.lng } : null, mode: participant.mode }))} tolerance={tolerance} available={!calculationUnavailable} onEvidenceChange={handleRouteFirstEvidence} /><div className="mx-auto flex min-h-screen max-w-[1500px] flex-col px-4 pb-6 pt-4 sm:px-6 lg:px-8">
    ...
    <div className="grid flex-1 gap-5 lg:grid-cols-[390px_minmax(0,1fr)]"><section className="order-2 flex flex-col lg:order-1"><div className="mb-5"><p className="mb-2 text-xs font-bold uppercase tracking-[.19em] text-[#d8644e]">Find the middle ground</p><h1 className="max-w-[320px] text-[2.35rem] font-semibold leading-[.98] tracking-[-.07em] sm:text-5xl">A fair place to meet.</h1><p className="mt-3 max-w-[330px] text-sm leading-6 text-[#6b716b]">Place everyone on the Munich map. We’ll find destinations within a similar travel time.</p></div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">{calculationUnavailable && <div className="rounded-2xl border border-[#cbd7cd] bg-[#e8f2eb] px-4 py-3 text-xs leading-5 text-[#315e4d]" role="status" aria-live="polite"><p className="font-bold text-[#165b47]">Routing foundation configured</p><p className="mt-0.5">{plannerUiState.unavailableMessage}</p></div>}{isDirectTransit && <div className="rounded-2xl border border-[#cbd7cd] bg-[#e8f2eb] px-4 py-3 text-xs leading-5 text-[#315e4d]"><p className="font-bold text-[#165b47]">MVG direct · public transport only</p><p className="mt-0.5">This connection uses MVG transit routes with realtime when supplied and scheduled time as the fallback, so every participant starts in public transport mode.</p></div>}{participants.map((participant, index) => <ParticipantRow key={participant.id} participant={participant} index={index} locationError={locationErrors[participant.id]} locationRef={(element) => { locationRefs.current[participant.id] = element; }} onChange={(next) => updateParticipant(index, next)} onRemove={() => removeParticipant(index)} canRemove={participants.length > 2} disabled={status === "loading"} supportedModes={supportedModes} showModeSelector={plannerUiState.showModeSelector} />)}
