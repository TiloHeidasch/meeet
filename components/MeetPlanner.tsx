...
function ParticipantRow({ participant, index, locationError, locationRef, onChange, onRemove, canRemove, disabled }: { participant: Participant; index: number; locationError?: string; locationRef: (element: HTMLSelectElement | null) => void; onChange: (next: Participant) => void; onRemove: () => void; canRemove: boolean; disabled: boolean }) {
...
export default function MeetPlanner() {
  ...
  return <main className="min-h-screen bg-[#f4f1eb] text-[#202522]"><AttributionSummary metadata={calculation?.response.metadata} /><div className="mx-auto flex min-h-screen max-w-[1500px] flex-col px-4 pb-6 pt-4 sm:px-6 lg:px-8">
    ...
    <div className="grid flex-1 gap-5 lg:grid-cols-[390px_minmax(0,1fr)]"><section className="order-2 flex flex-col lg:order-1"><div className="mb-5"><p className="mb-2 text-xs font-bold uppercase tracking-[.19em] text-[#d8644e]">Find the middle ground</p><h1 className="max-w-[320px] text-[2.35rem] font-semibold leading-[.98] tracking-[-.07em] sm:text-5xl">A fair place to meet.</h1><p className="mt-3 max-w-[330px] text-sm leading-6 text-[#6b716b]">Place everyone on the Munich map. We’ll find destinations within a similar travel time.</p></div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">{participants.map((participant, index) => <ParticipantRow key={participant.id} participant={participant} index={index} locationError={locationErrors[participant.id]} locationRef={(element) => { locationRefs.current[participant.id] = element; }} onChange={(next) => updateParticipant(index, next)} onRemove={() => removeParticipant(index)} canRemove={participants.length > 2} disabled={status === "loading"} />)}
        ...
        <div aria-live="polite" className={`min-h-6 text-center text-xs leading-5 ${status === "error" || status === "empty" ? "text-[#a64e39]" : "text-[#6b716b]"}`}>{message || "Local demo only: static venues, no MVG/MVV timetable or realtime data."}</div>
      </form></section>
