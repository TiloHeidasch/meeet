...
function LocationInput({ participant, index, error, disabled, onChange }: { participant: Participant; index: number; error?: string; disabled: boolean; onChange: (location: Location) => void }) {
  ...
  const listboxId = `location-results-${index + 1}`; return <fieldset className="origin-card"><legend className="sr-only">Participant {index + 1} origin</legend><div className="origin-title"><span className="origin-number" style={{ backgroundColor: COLORS[index] }}>{index + 1}</span><span>Participant {index + 1}</span><span className="origin-mode">Transit</span></div><label><span className="field-label">Starting point in Munich</span><input value={query} disabled={disabled} onChange={(event) => { setQuery(event.target.value); setResults([]); }} onKeyDown={keyDown} role="combobox" aria-label={`Participant ${index + 1} starting point`} aria-controls={listboxId} aria-expanded={results.length > 0} className={error ? "input-error" : ""} placeholder="Search a street, station, or place" autoComplete="off" />{participant.location && <span className="selected-origin"><Icon name="pin"/> {participant.location.label}</span>}{searching && <span className="input-hint">Looking within Munich…</span>}{error && <span className="input-error-text">{error}</span>}</label>{results.length > 0 && <ul id={listboxId} role="listbox" className="location-results">{results.map((result) => <li role="option" aria-selected="false" key={`${result.label}-${result.latitude}`}><button type="button" onClick={() => choose(result)}>{result.label}</button></li>)}</ul>}</fieldset>;
}
