...
export default function MapLibreCanvas({ participants, stationAreas, resultState, selectedStationAreaId = null, onStationAreaSelect }: Props) {
  ...
  useEffect(() => { const map = mapRef.current; if (!map || state !== "ready") return; setTooltip(null); setMarkersReady(false); const territorySource = map.getSource("meeet-territories") as GeoJSONSource | undefined; const stationSource = map.getSource("meeet-station-areas") as GeoJSONSource | undefined; territorySource?.setData({ type: "FeatureCollection", features: [] }); stationSource?.setData(stations); const generated = territoryData(stationAreas); territorySource?.setData(generated); setTerritoryFeatureCount(generated.features.length); syncOrigins(map, participants, markersRef); const markReady = () => { if (!map || map !== mapRef.current) return; const ready = stationAreas.length === 0 || stationAreas.every((area) => map.queryRenderedFeatures(map.project([area.coordinate.longitude, area.coordinate.latitude]), { layers: STATION_LAYERS }).some((feature) => feature.properties?.stationAreaId === area.stationAreaId)); if (ready) setMarkersReady(true); }; map.once("idle", markReady); return () => { map.off("idle", markReady); }; }, [stationAreas, stations, participants, state]);
