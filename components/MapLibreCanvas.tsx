...
export default function MapLibreCanvas({ participants, cells, onParticipantMove, resultState }: Props) {
  ...
  useEffect(() => {
    if (!containerRef.current) { setState("unavailable"); return; } let active = true; let map: Map | null = null; let hasCartography = false; let resizeObserver: ResizeObserver | null = null;
    try {
      map = new Map({ container: containerRef.current, style: styleUrl, center: MUNICH_CENTER, zoom: 11, attributionControl: false }); mapRef.current = map; resizeObserver = new ResizeObserver(() => map?.resize()); resizeObserver.observe(containerRef.current); map.addControl(new AttributionControl({ compact: true, customAttribution: attribution }), "bottom-right"); map.addControl(new NavigationControl({ showCompass: false }), "top-right");
      map.once("style.load", () => { if (!map || !active) return; hasCartography = map.getStyle().layers?.some((layer) => layer.type !== "background") ?? false; if (!hasCartography) { setState("unavailable"); return; } map.addSource("meeet-surface", { type: "geojson", data: cellsData([]) }); map.addLayer({ id: "meeet-surface-red", type: "fill", source: "meeet-surface", filter: ["==", ["get", "classification"], "red"], paint: { "fill-color": "#e85d4a", "fill-opacity": .52, "fill-outline-color": "#d84a3b" } }); map.addLayer({ id: "meeet-surface-blue", type: "fill", source: "meeet-surface", filter: ["==", ["get", "classification"], "blue"], paint: { "fill-color": "#3d70c9", "fill-opacity": .5, "fill-outline-color": "#315ca8" } }); map.addLayer({ id: "meeet-surface-fair", type: "fill", source: "meeet-surface", filter: ["==", ["get", "classification"], "fair"], paint: { "fill-color": "#f0ca43", "fill-opacity": .72, "fill-outline-color": "#d5ae25" } }); syncMarkers(map, inputs.current.participants, markersRef, (id, coordinate) => inputs.current.onParticipantMove(id, coordinate)); });
      map.once("idle", () => { if (active) { map?.resize(); setState(!hasCartography ? "unavailable" : "ready"); } }); map.on("error", () => { if (active) setState("unavailable"); });
    } catch { queueMicrotask(() => { if (active) setState("unavailable"); }); }
    return () => { active = false; resizeObserver?.disconnect(); markersRef.current.forEach((marker) => marker.remove()); markersRef.current.clear(); map?.remove(); mapRef.current = null; };
  }, [styleUrl, attribution]);
  useEffect(() => { const map = mapRef.current; if (!map || state !== "ready") return; (map.getSource("meeet-surface") as GeoJSONSource | undefined)?.setData(cellsData(cells)); syncMarkers(map, participants, markersRef, onParticipantMove); }, [cells, participants, onParticipantMove, state]);
  ...
  return <section className="map-frame" aria-label={label} data-map-state={state} data-map-style="configured"><h2 className="sr-only">Munich meeting map</h2><div ref={containerRef} className="absolute inset-0" style={{ width: "100%", height: "100%" }} />{state === "loading" && <div className="map-message" role="status">Loading Munich map…</div>}{state === "unavailable" && <div className="map-message" role="status"><strong>Map unavailable</strong><span>The meeting result is still available as a surface summary.</span></div>}</section>;
}
