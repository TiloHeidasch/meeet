...
export const LOCATIONS = {
  Marienplatz: { label: "Marienplatz", latitude: 48.1374, longitude: 11.5755, type: "STATION" },
  Ostbahnhof: { label: "Ostbahnhof", latitude: 48.1257, longitude: 11.605, type: "STATION" },
  Garching: { label: "Garching", latitude: 48.2614, longitude: 11.6711, type: "STATION" },
  Baldham: { label: "Baldham", latitude: 48.1014, longitude: 11.7872, type: "STATION" },
  Herrsching: { label: "Herrsching", latitude: 48.0025, longitude: 11.1764, type: "STATION" },
} as const;
...
export function detailsFixture(request: { contractVersion: "meeet-meeting/v3"; participants: Array<{ id: string; origin: { label: string; latitude: number; longitude: number }; mode: "transit" }>; tolerancePercent: 5 | 10 | 15; searchStartAt: string; changeTimePreset: "quick" | "medium" | "long" }, areaId: string) {
  ...
  const startEpoch = Math.floor(Date.parse(request.searchStartAt) / 1000);
  const walkSeconds = 300;
  const participants = ["red", "blue"].map((color, index) => { const total = color === "red" ? area.redArrivalSeconds : area.blueArrivalSeconds; const origin = request.participants[index]!.origin; if (total === null) return { id: request.participants[index]!.id, color, origin, status: "unavailable", unavailableReason: area.classification === "unclassified" ? "station-area-unclassified" : "station-area-unavailable-for-participant", terminal: { totalSeconds: null, arrivalAt: null }, itinerary: null }; const arrival = new Date(Date.parse(request.searchStartAt) + total * 1000).toISOString(); const itinerary = [{ kind: "walk", fromAreaId: null, toAreaId: "station-nearby", fromAreaName: null, toAreaName: "Nearby station", startEpochSeconds: startEpoch, endEpochSeconds: startEpoch + walkSeconds }, { kind: "transit", fromAreaId: "station-nearby", toAreaId: area.stationAreaId, fromAreaName: "Nearby station", toAreaName: area.name, line: "U3", routeType: 1, headsign: "Fürstenrieder Straße", tripId: `trip-${color}`, startEpochSeconds: startEpoch + walkSeconds, endEpochSeconds: startEpoch + total }]; return { id: request.participants[index]!.id, color, origin, status: "available", unavailableReason: null, terminal: { totalSeconds: total, arrivalAt: arrival }, itinerary }; });
