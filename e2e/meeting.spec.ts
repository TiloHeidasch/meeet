...
function v2Fixture(request: { arrivalAt: string; participants: Array<{ id: string; location: { label: string; latitude: number; longitude: number } }>; tolerancePercent: number }, noResult = false) {
  ...
  const routePatterns = noResult ? [] : [{ id: "fixture-origin-to-origin", kind: "transit", transitStops: discoveryStops, lines: [{ identity: "discovery-line", type: "BUS" }], parts: [{ kind: "transit", from: discoveryStops[0]!, to: discoveryStops[discoveryStops.length - 1]!, intermediateStops: discoveryStops.slice(1, -1), line: { identity: "discovery-line", type: "BUS" }, geometry: { type: "LineString", coordinates: [[discoveryStops[0]!.coordinate.longitude, discoveryStops[0]!.coordinate.latitude], [discoveryStops[discoveryStops.length - 1]!.coordinate.longitude, discoveryStops[discoveryStops.length - 1]!.coordinate.latitude]] }, plannedDepartureAt: new Date(arrival - 1_200_000).toISOString(), plannedArrivalAt: request.arrivalAt }], provenance: [{ direction: "participant-1-to-participant-2", searchKind: "direct", anchorStationGlobalId: null }] }];
  const journey = (participant: typeof request.participants[number], point: typeof points[number], duration: number, line: string, leg: string) => { const origin = endpoint({ latitude: participant.location.latitude, longitude: participant.location.longitude }, `origin-${participant.id}`, participant.location.label); const destination = endpoint(point.coordinate, point.stationGlobalId, point.label); const departure = new Date(arrival - duration).toISOString(); const access = endpoint(origin.coordinate, `walk-${participant.id}`, `${leg} access`); return { participantId: participant.id, mode: "transit", plannedDepartureAt: departure, plannedArrivalAt: request.arrivalAt, plannedDurationMilliseconds: duration, source: "local-test-fixture", origin, destination, parts: [{ kind: "walking", from: origin, to: access, intermediateStops: [], line: null, geometry: { type: "LineString", coordinates: [[origin.coordinate.longitude, origin.coordinate.latitude], [access.coordinate.longitude, access.coordinate.latitude]] }, plannedDepartureAt: departure, plannedArrivalAt: new Date(Date.parse(departure) + 120_000).toISOString() }, { kind: "transit", from: access, to: destination, intermediateStops: [endpoint(point.coordinate, `${point.stationGlobalId}-mid`, `${leg} interchange`)], line: { identity: line, type: "BUS" }, geometry: { type: "LineString", coordinates: [[access.coordinate.longitude, access.coordinate.latitude], [destination.coordinate.longitude, destination.coordinate.latitude]] }, plannedDepartureAt: new Date(Date.parse(departure) + 120_000).toISOString(), plannedArrivalAt: request.arrivalAt }] }; };
...
test.describe("canonical v2 meeting search", () => {
  ...
  test("submits arrivalAt and shows sampled station locations with effective tolerance", async ({ page }) => {
    ...
    const map = page.getByRole("region", { name: /Interactive Munich map showing .*sampled station locations/ });
    const journeys = page.locator("section[aria-labelledby^='journey-']");
    await journeys.first().hover();
    await expect(map).toHaveAttribute("data-focused-journey-parts", "2");
    await page.mouse.move(10, 10);
    await expect(map).toHaveAttribute("data-focused-journey-parts", "0");
    await journeys.first().focus();
    await expect(map).toHaveAttribute("data-focused-journey-parts", "2");
