...
function v3Fixture(request: { contractVersion: "meeet-meeting/v3"; participants: Array<{ id: string; origin: { label: string; latitude: number; longitude: number }; mode: "transit" }>; tolerancePercent: 5 | 10 | 15; searchStartAt: string }, reason: "ok" | "no-access-seeds" = "ok") {
  const stationAreas = [
    { stationAreaId: "area-red", name: "Red area", coordinate: { latitude: 48.132, longitude: 11.555 }, redBoardingStopId: "red-stop", blueBoardingStopId: null, classification: "red" as const, redArrivalSeconds: 1200, blueArrivalSeconds: null, fasterParticipant: "red" as const, withinSelectedTolerance: false },
    { stationAreaId: "area-fair", name: "Fair area", coordinate: { latitude: 48.132, longitude: 11.585 }, redBoardingStopId: "fair-red-stop", blueBoardingStopId: "fair-blue-stop", classification: "fair" as const, redArrivalSeconds: 1800, blueArrivalSeconds: 1860, fasterParticipant: "red" as const, withinSelectedTolerance: true },
    { stationAreaId: "area-blue", name: "Blue area", coordinate: { latitude: 48.132, longitude: 11.615 }, redBoardingStopId: null, blueBoardingStopId: "blue-stop", classification: "blue" as const, redArrivalSeconds: null, blueArrivalSeconds: 1300, fasterParticipant: "blue" as const, withinSelectedTolerance: false },
    { stationAreaId: "area-unclassified", name: "Unclassified area", coordinate: { latitude: 48.14, longitude: 11.59 }, redBoardingStopId: null, blueBoardingStopId: null, classification: "unclassified" as const, redArrivalSeconds: null, blueArrivalSeconds: null, fasterParticipant: null, withinSelectedTolerance: false },
  ];
  ...
  const surface = { contractVersion: "meeet-scheduled-routing/v1", scheduleContentHash: schedule.scheduleContentHash, compiledArtifactId: schedule.compiledArtifactId, feedId: schedule.feedId, timeZone: schedule.timeZone, searchStartAt: request.searchStartAt, routingHorizonSeconds: 86400, selectedTolerancePercent: request.tolerancePercent, walkingVelocityMetersPerSecond: 1.4, walkingSecondsRoundingRule: "ceil(distanceMetres / velocityMetresPerSecond), with zero distance taking zero seconds", transferRadiusMeters: 100, accessSeedCounts: reason === "no-access-seeds" ? [0, 0] as [number, number] : [1, 1] as [number, number], stationAreaCount: 2, boardingStopCount: 2, connectionCount: 2, coverage: "scheduled-service-day-local-radius/v1" as const, representativePointBasis: "inside-clipped-cell/v1" as const, classificationMethod: "representative-point-with-geometric-final-station-walking/v1" as const, classificationBasis: "representative-point" as const, finalWalkingMethod: "geometric-station-walking-estimate-not-navigation" as const };
  ...
  return { contractVersion: "meeet-meeting/v3" as const, status: reason === "ok" ? "ok" as const : "no-result" as const, reason: reason === "ok" ? null : reason, participants: [{ id: request.participants[0]!.id, color: "red" as const, origin: request.participants[0]!.origin, mode: "transit" as const, accessSeeds: reason === "no-access-seeds" ? [] : [seed("station-red", { latitude: 48.137, longitude: 11.576 })] }, { id: request.participants[1]!.id, color: "blue" as const, origin: request.participants[1]!.origin, mode: "transit" as const, accessSeeds: reason === "no-access-seeds" ? [] : [seed("station-blue", { latitude: 48.126, longitude: 11.605 })] }], stationAreas: reason === "ok" ? stationAreas : stationAreas.map((area) => ({ ...area, classification: "unclassified" as const, redBoardingStopId: null, blueBoardingStopId: null, redArrivalSeconds: null, blueArrivalSeconds: null, fasterParticipant: null, withinSelectedTolerance: false })), metadata: { schedule, surface, accessProvider, stationAreas: { count: 4, coverage: "official-munich-boundary-with-connected-artifact-boarding-stops/v1" as const, selection: "all-eligible-scheduled-station-areas/v1" as const }, coverage: "munich-clipped-scheduled-grid/v1" as const } };
}
function detailsFixture(request: { contractVersion: "meeet-meeting/v3"; participants: Array<{ id: string; origin: { label: string; latitude: number; longitude: number }; mode: "transit" }>; tolerancePercent: 5 | 10 | 15; searchStartAt: string }, areaId: string) {
  ...
  const endpoint = (id: string, name: string) => ({ boardingStopId: id, stationAreaId: area.stationAreaId, name });
  const participants = ["red", "blue"].map((color, index) => { const stop = color === "red" ? area.redBoardingStopId : area.blueBoardingStopId; const total = color === "red" ? area.redArrivalSeconds : area.blueArrivalSeconds; const origin = request.participants[index]!.origin; if (stop === null || total === null) return { id: request.participants[index]!.id, color, origin, status: "unavailable", unavailableReason: area.classification === "unclassified" ? "station-area-unclassified" : "station-area-unavailable-for-participant", terminal: { boardingStopId: null, totalSeconds: null, arrivalAt: null }, segments: [] }; const walkSeconds = 72; const lat = (area.coordinate.latitude - origin.latitude) * Math.PI / 180; const lon = (area.coordinate.longitude - origin.longitude) * Math.PI / 180; const a = Math.sin(lat / 2) ** 2 + Math.cos(origin.latitude * Math.PI / 180) * Math.cos(area.coordinate.latitude * Math.PI / 180) * Math.sin(lon / 2) ** 2; const walkEnd = new Date(Date.parse(request.searchStartAt) + walkSeconds * 1000).toISOString(); const arrival = new Date(Date.parse(request.searchStartAt) + total * 1000).toISOString(); return { id: request.participants[index]!.id, color, origin, status: "available", unavailableReason: null, terminal: { boardingStopId: stop, totalSeconds: total, arrivalAt: arrival }, segments: [{ kind: "walk", purpose: "origin-access", durationSeconds: walkSeconds, distanceMeters: 2 * 6_371_000 * Math.asin(Math.sqrt(Math.min(1, a))), from: { latitude: origin.latitude, longitude: origin.longitude }, to: area.coordinate, estimate: "geometric-estimate-not-directions/v1", startAt: request.searchStartAt, endAt: walkEnd }, { kind: "identity-resolution", purpose: "station-access", durationSeconds: 0, startAt: walkEnd, endAt: walkEnd, source: "mvg-nearby-to-mvv-gtfs-identity/v1", target: "station-area", from: area.coordinate, to: { stationAreaId: area.stationAreaId, name: area.name }, toCoordinate: area.coordinate }, { kind: "transit", durationSeconds: total - walkSeconds, startAt: walkEnd, endAt: arrival, source: "mvv-gtfs", serviceDate: "2026-08-15", serviceId: `service-${color}`, tripId: `trip-${color}`, line: color === "red" ? "U3" : "U4", headsign: "Munich", from: endpoint(stop, `${color} boarding stop`), to: endpoint(stop, `${color} boarding stop`) }] }; });
  return { contractVersion: "meeet-station-area-details/v1", status: "ok", reason: null, stationArea: area, participants, basis: { contractVersion: "meeet-meeting/v3", searchStartAt: request.searchStartAt, selectedTolerancePercent: request.tolerancePercent, routingHorizonSeconds: 86400, walkingVelocityMetersPerSecond: 1.4, walkingSecondsRoundingRule: "ceil(distanceMetres / velocityMetresPerSecond), with zero distance taking zero seconds", transferRadiusMeters: 100, deterministicSelectionPolicy: "earliest-arrival/canonical-scan-first/v1", schedule, accessProvider: fixture.metadata.accessProvider } };
}
...
test.describe("v3 Munich meeting surface", () => {
  ...
  test("selects every station area with native keyboard and exposes participant legs", async ({ page }) => { await setup(page); await openPlanner(page); await selectOrigin(page, 0, "Marienplatz"); await selectOrigin(page, 1, "Ostbahnhof"); await page.getByRole("button", { name: "Show meeting surface" }).click(); for (const id of ["area-red", "area-blue", "area-fair", "area-unclassified"]) { const area = page.locator(`[data-station-area-id="${id}"]`); await area.focus(); await page.keyboard.press("Enter"); await expect(area).toHaveAttribute("aria-pressed", "true"); await expect(page.getByRole("heading", { name: "Station-area details" })).toBeVisible(); } const fair = page.locator('[data-station-area-id="area-fair"]'); await fair.focus(); await page.keyboard.press("Enter"); await expect(page.getByRole("heading", { name: "Participant 1" })).toBeVisible(); await expect(page.getByText(/Transit U3/)).toBeVisible(); await expect(page.getByText(/Access walk from your start/).first()).toBeVisible(); await page.getByText("Schedule and access provenance", { exact: true }).click(); await expect(page.locator(".detail-provenance-copy")).toContainText("fixture"); await expect(page.locator(".detail-provenance-copy")).toContainText("MVV"); await expect(page.locator(".detail-provenance-copy")).toContainText("Fixture"); await page.keyboard.press("Space"); await expect(fair).toHaveAttribute("aria-pressed", "true"); await expect(page.locator('[data-station-area-id="area-unclassified"]')).toHaveCount(1); });
  test("selects unclassified areas with explicit unavailable participant details", async ({ page }) => { await setup(page, "no-access-seeds"); await openPlanner(page); await selectOrigin(page, 0, "Marienplatz"); await selectOrigin(page, 1, "Ostbahnhof"); await page.getByRole("button", { name: "Show meeting surface" }).click(); const unclassified = page.locator('[data-station-area-id="area-unclassified"]'); await unclassified.click(); await expect(unclassified).toHaveAttribute("aria-pressed", "true"); await expect(page.getByRole("heading", { name: "Participant 1" })).toBeVisible(); await expect(page.getByRole("heading", { name: "Participant 2" })).toBeVisible(); await expect(page.getByText(/No nearby access seed was available/)).toHaveCount(2); });
  test("activates a station marker through the real MapLibre canvas pointer", async ({ page }) => { await setup(page); await openPlanner(page); await selectOrigin(page, 0, "Marienplatz"); await selectOrigin(page, 1, "Ostbahnhof"); await page.getByRole("button", { name: "Show meeting surface" }).click(); await expect(page.locator('.map-frame[data-map-state="ready"]')).toHaveAttribute("data-station-markers-ready", "true"); const canvas = page.locator(".maplibregl-canvas"); const point = await page.evaluate(() => { const map = (window as unknown as { __meeetMap?: { project: (coordinate: [number, number]) => { x: number; y: number } } }).__meeetMap; if (!map) throw new Error("Map instance unavailable"); return map.project([11.585, 48.132]); }); const rect = await canvas.boundingBox(); if (!rect) throw new Error("Map canvas was not painted"); const request = page.waitForRequest((item) => item.url().includes("/api/meeting/station-areas/area-fair/details")); await page.mouse.click(rect.x + point.x, rect.y + point.y); await request; const fair = page.locator('[data-station-area-id="area-fair"]'); await expect(fair).toHaveAttribute("aria-pressed", "true"); await expect(page.locator(".station-detail-panel")).toContainText("Fair area"); await expect(page.locator(".station-detail-panel")).toContainText("30 min"); });
  ...
  test("clears the hover tooltip when the calculation becomes stale", async ({ page }) => {
    await setup(page); await openPlanner(page); await selectOrigin(page, 0, "Marienplatz"); await selectOrigin(page, 1, "Ostbahnhof"); await page.getByRole("button", { name: "Show meeting surface" }).click(); await expect(page.locator('.map-frame[data-map-state="ready"]')).toHaveAttribute("data-station-markers-ready", "true");
    const tooltip = page.locator("[data-station-tooltip]"); const canvas = page.locator(".maplibregl-canvas"); const rect = await canvas.boundingBox(); if (!rect) throw new Error("Map canvas was not painted");
    const project = (lng: number, lat: number) => page.evaluate((coordinate) => { const map = (window as unknown as { __meeetMap?: { project: (coordinate: [number, number]) => { x: number; y: number } } }).__meeetMap; if (!map) throw new Error("Map instance unavailable"); return map.project(coordinate); }, [lng, lat] as [number, number]);
    const fair = await project(11.585, 48.132); await page.mouse.move(rect.x + fair.x, rect.y + fair.y); await expect(tooltip).toBeVisible(); await expect(tooltip).toContainText("Fair area");
    ...
    await page.getByRole("button", { name: "Show meeting surface" }).click(); await expect(page.locator(".map-frame")).toHaveAttribute("data-station-area-count", "4"); await expect(page.locator('.map-frame[data-map-state="ready"]')).toHaveAttribute("data-station-markers-ready", "true");
    await page.mouse.move(rect.x + fair.x, rect.y + fair.y); await expect(tooltip).toBeVisible(); await expect(tooltip).toContainText("Fair area");
    const away = await project(11.63, 48.145); await page.mouse.move(rect.x + away.x, rect.y + away.y); await expect(tooltip).toHaveCount(0);
  });
  ...
  test("cancel returns to idle and preserves inputs", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Show meeting surface" })).toBeVisible();
