...
async function setup(page: Page, noResult = false) {
  ...
  await page.route("**/api/meeting/calculate", async (route) => { const request = route.request().postDataJSON() as Parameters<typeof v2Fixture>[0]; await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(v2Fixture(request, noResult)) }); });
}
...
test.describe("canonical v2 meeting search", () => {
  ...
  test("submits arrivalAt and shows sampled station locations with effective tolerance", async ({ page }) => {
    ...
    await expect(page.getByText(/Sampled transit stations/)).toBeVisible();
    await expect(page.getByText(/How meeet searched these stations/)).toBeVisible();
    await page.getByText("How meeet searched these stations", { exact: true }).click();
    await expect(page.getByText(/does not assess skipped stations or prove complete fair-location coverage/)).toBeVisible();
  ...
  test("treats no station targets as a successful empty result", async ({ page }) => {
  ...
  test("rejects an arrival outside the Berlin planning window without requesting a calculation", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Find fair locations" }).click();
    await expect(page.getByText(/from now through the end of tomorrow in Berlin/)).toBeVisible();
